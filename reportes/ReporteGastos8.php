...
class ReporteHorario extends FPDF
{
	...
	{	
		...
		$this->SetFont('Arial','B',7.5);
		$this->SetXY(86.5,24);
		$this->Cell(55,4,"DEPARTAMENTO : ".$row_dpto['abreviatura'] ,0,2,'L');
	...
	function parteII()
	{
		function PresupuestoCapitulo($dpto_id,$medida,$bdd,$tamano)
		{
			...
			$gastoDpto = 0;
			$tamano1=$tamano+1000;	
			$sql_poa_dpto_gastos = "SELECT poa_dpto_gastos.id AS poa_id, poa_dpto_gastos.*, insumo.*, partida.* FROM poa_dpto_gastos, insumo, partida WHERE poa_dpto_gastos.dpto_id != 0 AND poa_dpto_gastos.insumo_id = insumo.id AND poa_dpto_gastos.partida_id = partida.id AND poa_dpto_gastos.idacciones='$medida' AND partida.clavepartida>='{$tamano}' AND partida.clavepartida<'{$tamano1}' AND poa_dpto_gastos.tipogasto=1";
		...
		$sql_programa = "SELECT * FROM poa_dpto_gastos WHERE tipogasto=1 GROUP BY idacciones ";
		if ( $res_programa = mysql_db_query ( $bdd, $sql_programa ) )
		{
			while ( $row_programa = mysql_fetch_assoc ( $res_programa ) )
			{
				...
				{
					$totalgastos=0;
					$sql_gastos_dpto = "SELECT * FROM gastos_dpto WHERE idmeta = '{$row_programa['idacciones']}' AND donde != 'Remanente' AND donde != 'Cancelado'";
								...
								if ( $row_partida_busqueda = mysql_fetch_assoc ( $res_partida_busqueda ) )
								{
									if(($row_partida_busqueda['clavepartida'] >= 1000) and ($row_partida_busqueda['clavepartida'] <= 1999))
									{
										...
										if ( $row_1 = mysql_fetch_assoc ( $res_1 ) )
										{
											...
											$this->SetFillColor(255,255,255);
											$this->SetXY(140,$x);
											$this->Cell(60,4,"{$row_1['abreviatura']}",1,2,'C',1);
										}
										if($x>260)
										{
											$x=37;
				...
				$tamano=1000;
				$tamano1=2000;	
				$sql_poa_dpto_gastos = "SELECT poa_dpto_gastos.id AS poa_id, poa_dpto_gastos.*, insumo.*, partida.* FROM poa_dpto_gastos, insumo, partida WHERE poa_dpto_gastos.insumo_id = insumo.id AND poa_dpto_gastos.partida_id = partida.id AND poa_dpto_gastos.idacciones='{$row_programa['idacciones']}' AND partida.clavepartida>='{$tamano}' AND partida.clavepartida<'{$tamano1}' AND poa_dpto_gastos.tipogasto=1";
				if ( $res_poa_dpto_gastos = mysql_db_query ( $bdd, $sql_poa_dpto_gastos ) )
				{
					while ( $row_poa_dpto_gastos = mysql_fetch_assoc ( $res_poa_dpto_gastos ) )
					{
