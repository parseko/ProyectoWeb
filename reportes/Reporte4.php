...
class ReporteHorario extends FPDF
{
	...
	function parteII()
	{
		...
		$sql_capitulo5000 = "SELECT partida.clavepartida, partida.id, insumo.descinsu, poa_dpto.cantidad, poa_dpto.idacciones, poa_dpto.idactividad, insumo.costuni, poa_dpto.justificacion, dpto.nombredpto FROM partida, insumo, poa_dpto, dpto WHERE partida.id = poa_dpto.partida_id AND poa_dpto.insumo_id = insumo.id AND poa_dpto.dpto_id = dpto.id AND clavepartida >5000 AND clavepartida <6000 GROUP BY poa_dpto.idactividad ";
		$res_capitulo5000 = mysql_db_query ( $bdd, $sql_capitulo5000 );
		while ( $row_capitulo5000 = mysql_fetch_assoc ( $res_capitulo5000 ) )
		{
			$sql_capitulo5000a = "SELECT partida.clavepartida, insumo.descinsu, poa_dpto.cantidad, poa_dpto.idacciones, poa_dpto.idproceso, poa_dpto.idactividad, insumo.costuni, poa_dpto.justificacion, poa_dpto.dpto_id, dpto.nombredpto, dpto.abreviatura FROM partida, insumo, poa_dpto, dpto WHERE partida.id = poa_dpto.partida_id AND poa_dpto.insumo_id = insumo.id AND poa_dpto.dpto_id = dpto.id AND clavepartida >5000 AND clavepartida <6000 AND poa_dpto.idactividad= '{$row_capitulo5000['idactividad']}' ORDER BY partida.clavepartida";
	...
	function Footer()
	{
		...
		$this->SetXY(10,$x+21);
		$this->Cell(10,4,$row_revision['snest2'],0,2,'C',1);
