...
class ReporteHorario extends FPDF
{
	...
	{	
		...
		$this->SetFont('Arial','B',7.5);
		$this->SetXY(105,28);
		$this->Cell(45,4,"CONCENTRADO POR PROCESO CLAVE Y ESTRATEGICO",0,2,'L');
	...
	function parteII()
	{
		...
		while ( $row_proceso = mysql_fetch_assoc ( $res_proceso ) )
		{
			...
			while ( $row_actividad = mysql_fetch_assoc ( $res_actividad ) )
			{
				...
				$sql_apoa = "SELECT * FROM poa_dpto WHERE idpoa = '{$row_apoa['id']}' AND idactividad = '{$row_actividad['id']}' ";
				$res_apoa = mysql_db_query ( $bdd, $sql_apoa );
			...
			if ( $row_apoa = mysql_fetch_assoc ( $res_apoa ) )
			{}
			$sql_apoa_pro = "SELECT * FROM poa_dpto WHERE idpoa = '{$row_apoa['id']}' AND idproceso = '{$row_proceso['id']}' ";
