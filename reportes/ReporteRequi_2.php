...
//	    $this->Image("../imagenes/reportes/tec.jpeg",255,3,25,25);
		...
		$bdd="sicopre";
		$sql_requi = "SELECT * FROM solicitud_servicio WHERE idsolicitud = '{$_SESSION['Oficio']}'";
	...
	function parteII()
	{
		$x=68;
		$bdd="sicopre";
		$sql_bien = "SELECT * FROM solicitud_servicio WHERE idsolicitud = '{$_SESSION['Oficio']}'";
		$res_bien = mysql_db_query ( $bdd, $sql_bien );
		while ( $row_bien = mysql_fetch_assoc ( $res_bien ) )
		{
			$sql_meta = "SELECT * FROM accion WHERE id = '{$row_bien['idmeta']}'";
			...
			{}
			$sql_partida = "SELECT * FROM partida WHERE id = '{$row_bien['idpartida']}'";
			...
			$this->SetFont('Arial','B',7.5);
			$this->SetXY(116,$x);
			//$this->Cell(134,4,$row_bien['descripcion'],0,2,'C',1);
			...
			$this->Cell(35,4,"$".number_format ($row_bien['importe'], 2, '.', ',' ),0,2,'C',1);
			$_SESSION['Total']=$row_bien['importe'];
	...
	function Footer()
	{
		...
		$sql_requi = "SELECT * FROM solicitud_servicio WHERE idsolicitud = '{$_SESSION['Oficio']}' and planea = 1 ";
		$res_requi = mysql_db_query ( $bdd, $sql_requi );
		while ( $row_requi = mysql_fetch_assoc ( $res_requi ) )
		{
			...
			{}
			$sql_pro1 = "SELECT * FROM actividad WHERE id = '{$row_pro['actividad_id']}' ";
			$res_pro1 = mysql_db_query ( $bdd, $sql_pro1 );
			if ( $row_pro1 = mysql_fetch_assoc ( $res_pro1 ) )
			{}

			$sql_pro2 = "SELECT * FROM proceso WHERE id = '{$row_pro1['proceso_id']}' ";
			$res_pro2 = mysql_db_query ( $bdd, $sql_pro2 );
			if ( $row_pro2 = mysql_fetch_assoc ( $res_pro2 ) )
			{}
			$this->SetFont('Arial','B',7.5);
			$this->SetXY(103,$x+5.5);
			$this->Cell(17,3.5,"Proc. Clav.: ".$row_pro2['claveproceso'],0,2,'L',1);
			
			$this->SetFont('Arial','B',7.5);
			$this->SetXY(103,$x+9);
			$this->Cell(15,4,"Proc. Est.: ".$row_pro1['claveActiv'],0,2,'L',1);
			...
			$sql_pro = "SELECT * FROM gastos_dpto WHERE idsolicitud = '{$_SESSION['Oficio']}' ";
			$res_pro = mysql_db_query ( $bdd, $sql_pro );
			if ( $row_pro = mysql_fetch_assoc ( $res_pro ) )
			{}
			$this->SetFont('Arial','B',7.5);
