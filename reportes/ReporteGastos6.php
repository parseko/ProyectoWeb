...
class ReporteHorario extends FPDF
{
	...
	{	
		...
		$this->SetFillColor(225,225,225);
		$this->SetXY(80,$x);
		$this->Cell(30,4,"EJERCIDO",1,2,'C',1);
	...
	function parteII()
	{
		...
		while ( $row_dpto = mysql_fetch_assoc ( $res_dpto ) )
		{
			...
			$sql_poa_dpto_gastos = "SELECT * FROM poa_dpto_gastos, insumo WHERE poa_dpto_gastos.dpto_id='{$row_dpto['id']}' AND poa_dpto_gastos.tipogasto=1 AND insumo.id=poa_dpto_gastos.insumo_id";
