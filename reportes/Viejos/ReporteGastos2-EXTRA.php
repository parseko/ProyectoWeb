...
class ReporteHorario extends FPDF
{
	...
	{	
		...
		{
			$sql_apoa_dpto = "SELECT * FROM gastos_poa_dpto  WHERE idpoa = '{$row_apoa['id']}' AND  idproceso = '{$row_proceso['id']}' GROUP BY idactividad";
			...
			$sql_apoa_dpto = "SELECT * FROM gastos_poa_dpto  WHERE idpoa = '{$row_apoa['id']}' AND idproceso = '{$row_proceso['id']}' GROUP BY idaccion";
			$res_apoa_dpto = mysql_db_query ( $bdd, $sql_apoa_dpto );
			while ( $row_apoa_dpto = mysql_fetch_assoc ( $res_apoa_dpto ) )
			{
				if($row_proceso['id'] == 1)
		...
		if ( $row_apoa = mysql_fetch_assoc ( $res_apoa ) )
		{
			...
			while ( $row_partida = mysql_fetch_assoc ( $res_partida ) )
			{
				if($row_partida['clavepartida'] >= 1000 and $row_partida['clavepartida'] <= 1999)
				{
					...
					$administracion1="";
					//{$_SESSION['dpto_id']}
					$sql_deptoapoa = "SELECT * FROM gastos_poa_dpto  WHERE partida_id='{$row_partida['id']}' AND idpoa='{$row_apoa['id']}' ";
