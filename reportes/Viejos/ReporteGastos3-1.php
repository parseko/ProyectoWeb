...
class ReporteHorario extends FPDF
{
	...
	{	
	    ...
	    $this->Image("../imagenes/reportes/dgest.JPG",10,6.5,42,19);
	...
	function parteII()
	{
		...
		$sql_programa = "SELECT * FROM poa_dpto WHERE dpto_id='{$_SESSION['DPTO']}' and tipogasto=1 GROUP BY idacciones ";
		...
		$GASTOS7000=0;
				...
				$tamano=1000;
				$tamano1=2000;	
				$sql_poa_dpto = "SELECT poa_dpto.id AS poa_id, poa_dpto.*, insumo.*, partida.* FROM poa_dpto, insumo, partida WHERE poa_dpto.dpto_id = '{$_SESSION['DPTO']}' AND poa_dpto.insumo_id = insumo.id AND poa_dpto.partida_id = partida.id AND poa_dpto.idacciones='{$res_programa['idacciones']}' AND partida.clavepartida>='{$tamano}' AND partida.clavepartida<'{$tamano1}' AND poa_dpto.tipogasto=1 ";
				if ( $res_poa_dpto = mysql_db_query ( $bdd, $sql_poa_dpto ) )
				{
					$totalUnidad = 0;
					$totalgastos = 0;
				...
				$tamano=2000;
				$tamano1=3000;	
				$sql_poa_dpto = "SELECT poa_dpto.id AS poa_id, poa_dpto.*, insumo.*, partida.* FROM poa_dpto, insumo, partida WHERE poa_dpto.dpto_id = '{$_SESSION['DPTO']}' AND poa_dpto.insumo_id = insumo.id AND poa_dpto.partida_id = partida.id AND poa_dpto.idacciones='{$row_programa['idacciones']}' AND partida.clavepartida>='{$tamano}' AND partida.clavepartida<'{$tamano1}' AND poa_dpto.tipogasto=1";
				if ( $res_poa_dpto = mysql_db_query ( $bdd, $sql_poa_dpto ) )
				{
					while ( $row_poa_dpto = mysql_fetch_assoc ( $res_poa_dpto ) )
					{
