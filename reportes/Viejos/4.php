<?php
	...
	while ( $row_partida = mysql_fetch_assoc ( $res_partida ) )
	{
		...
		while ( $row_proceso = mysql_fetch_assoc ( $res_proceso ) )
		{
			...
			$sql_ingreso =	"SELECT proceso.proyecto, proceso.claveproceso, partida.clavepartida, partida.descpartida, ( poa_dpto.cantidad * insumo.costuni ) AS gasto
									FROM proceso, actividad, accion, unidadmedida, poa_dpto, partida, insumo
									...
									AND poa_dpto.partida_id = partida.id
									AND poa_dpto.insumo_id = insumo.id
									AND claveproceso = {$row_proceso['claveproceso']}
									AND clavepartida = {$row_partida['clavepartida']}
									ORDER BY claveproceso, clavepartida";
			...
			$sql_operativo = 	"SELECT proceso.proyecto, proceso.claveproceso, partida.clavepartida, partida.descpartida, gob_federal.presupuesto
										FROM proceso, actividad, accion, unidadmedida, partida, gob_federal
										WHERE proceso.id = actividad.proceso_id
										AND actividad.id = accion.actividad_id
										AND accion.id = unidadmedida.accion_id
										AND unidadmedida.id = gob_federal.unidadmedida_id
										AND gob_federal.partida_id = partida.id
										AND claveproceso = {$row_proceso['claveproceso']}
										AND clavepartida = {$row_partida['clavepartida']}
										ORDER BY claveproceso, clavepartida";
