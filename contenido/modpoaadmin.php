<?php 
	...
	if ( $row_poa = mysql_fetch_assoc ( $res_poa ) )
	{
		if ( $row_poa['iniciado'] == 1 and $row_poa['terminado'] == 0 )
		{
			...
			{
				$sql_dpto = "SELECT * FROM presupuesto WHERE dpto_id = {$_GET['dpto_id']}";
				...
				$sql_poa = "SELECT poa_dpto.id AS poa_id, poa_dpto.*, insumo.* FROM poa_dpto, insumo WHERE poa_dpto.id = {$_POST['poa_dpto']} AND poa_dpto.dpto_id = {$_GET['dpto_id']} AND poa_dpto.unidadmedida_id = {$_POST['unidadmedida_id']} AND poa_dpto.insumo_id = insumo.id";
				//$sql_poa = "SELECT * FROM poa_dpto WHERE id = {$_POST['poa_dpto']}";
				$res_poa = mysql_db_query ( $bdd, $sql_poa );
				$row_poa = mysql_fetch_assoc ( $res_poa );
				
				
				$gastado = totalGastado( $_GET['dpto_id'], $bdd );
				...
				{
					$sql = "UPDATE poa_dpto SET dpto_id = {$_GET['dpto_id']}, unidadmedida_id = {$_POST['unidadmedida_id']}, partida_id = {$_POST['partida']}, insumo_id = {$_POST['insumo']}, cantidad = {$_POST['cantidad']}, justificacion = '{$_POST['justificacion']}' WHERE id = {$_POST['poa_dpto']}";
			...
			if ( $_POST or $_GET['mod'] == 1)
			{
				...
				if ( isset ( $_GET['poa_dpto'] ) and !isset ( $_POST['poa_dpto'] ) )
				{
					...
					$_POST['unidadmedida_id'] = $row_poa['unidadmedida_id'];
				}
				
				if ( isset ( $_POST['unidadmedida_id'] ) )
				{
					$sql_unidad = "SELECT * FROM unidadmedida WHERE id = '{$_POST['unidadmedida_id']}'";
								...
								if ( isset ( $errorCapturaPoa ) )
								{
									switch ( $errorCapturaPoa )
									{
										...
										case 3:		echo "<tr><td colspan='100%'><div align = \"center\" class = \"MedianoAlerta\">Debe ingresar únicamente números en la cantidad de insumos.</div></td></tr>";
														break;
										case 4:		echo "<tr><td colspan='100%'><div align = \"center\" class = \"MedianoAlerta\">El total de los insumos pedido sobrepasa la cantidad de dinero restante en el departamento.</div></td></tr>";
														break;
									}
								}
			...
			?>
								<tr>
									...
									<?php	comboDinamico( "partida", "clavepartida", "SELECT * FROM partida WHERE estado = 1 ORDER BY clavepartida", $bdd ); ?>
										<input type="hidden" name="partida_hidden" value="<?php echo $_POST['partida']; ?>" />
									</td>
								</tr>
								<tr>
			...
			<?php
									if ( $_POST['partida']  != 0)
									{            			
										comboDinamico( "insumo", "descinsu", "SELECT * FROM insumo WHERE estado = 1 AND partida_id = {$_POST['partida']}", $bdd );
										$sql_insumo = "SELECT * FROM insumo WHERE id = {$_POST['insumo']}";
										$res_insumo = mysql_db_query ( $bdd, $sql_insumo );
										$row_insumo = mysql_fetch_assoc ( $res_insumo );
									}
			...
			<?php
									echo number_format ( totalGastado ( $_GET['dpto_id'], $bdd ), 2, '.', ',' );
			?>
									</td>
									...
									<input type="submit" name="modificar"  value="Modificar"/><input type="hidden" name="poa_dpto" value="<?php echo $_POST['poa_dpto']; ?>" />
									</td>
								</tr>
								<tr>
									<td colspan="4" bgcolor="#006699" class="MedianoAzulOscuro">&nbsp;</td>
		...
		<?php
			...
			else
			{
				...
				$sql_programa = "SELECT unidadmedida.id AS unidadMedida, unidadmedida.tipounidad FROM proceso, actividad, accion, unidadmedida WHERE unidadmedida.id  IN (SELECT unidadmedida_id FROM programa WHERE dpto_id = {$_GET['dpto_id']} ) AND accion.id = unidadmedida.accion_id AND actividad.id = accion.actividad_id AND proceso.id = actividad.proceso_id";
				if ( $res_programa = mysql_db_query ( $bdd, $sql_programa ) )
				{
					while ( $row_programa = mysql_fetch_assoc ( $res_programa ) )
					{
						...
						echo "<table width='96%' align='center'  border='1px'>";
						echo "<tr>";
						echo "<td bgcolor='#FFFFFF' class='MedianoAzulOscuro' colspan='100%' align='center'><strong>{$row_programa['tipounidad']}</strong></td>";
						echo "</tr>";
						echo "<tr>";
						echo "<td bgcolor='#006699' class='PequenioBlanco'><strong>Cantidad</strong></td>";
						echo "<td bgcolor='#006699' class='PequenioBlanco'><strong>Medida</strong></td>";
						echo "<td bgcolor='#006699' class='PequenioBlanco'><strong>Insumo</strong></td>";
						echo "<td bgcolor='#006699' class='PequenioBlanco'><strong>Costo unitario</strong></td>";
						echo "<td bgcolor='#006699' class='PequenioBlanco'><strong>Total</strong></td>";
						echo "<td bgcolor='#006699' class='PequenioBlanco'><strong>Justificacion</strong></td>";
						echo "<td bgcolor='#006699' class='PequenioBlanco'>&nbsp;</td>";
						echo "<td bgcolor='#006699' class='PequenioBlanco'>&nbsp;</td>";
						echo "</tr>";
						
						$sql_poa_dpto = "SELECT poa_dpto.id AS poa_id, poa_dpto.*, insumo.* FROM poa_dpto, insumo WHERE poa_dpto.dpto_id = {$_GET['dpto_id']} AND poa_dpto.unidadmedida_id = {$row_unidad['id']} AND poa_dpto.insumo_id = insumo.id";
						if ( $res_poa_dpto = mysql_db_query ( $bdd, $sql_poa_dpto ) )
						{
							while ( $row_poa_dpto = mysql_fetch_assoc ( $res_poa_dpto ) )
							{
								...
								echo "<tr>";
								echo "<td bgcolor='#FFFFFF' class='PequenioAzulOscuro'>{$row_poa_dpto['cantidad']}&nbsp;</td>";
								echo "<td bgcolor='#FFFFFF' class='PequenioAzulOscuro'>{$row_poa_dpto['medida']}&nbsp;</td>";
								echo "<td bgcolor='#FFFFFF' class='PequenioAzulOscuro'>{$row_poa_dpto['descinsu']}&nbsp;</td>";
								echo "<td bgcolor='#FFFFFF' class='PequenioAzulOscuro'>{$row_poa_dpto['costuni']}&nbsp;</td>";
								echo "<td bgcolor='#FFFFFF' class='PequenioAzulOscuro'>\$".number_format ( $total, 2, '.', ',' )."</td>";
								echo "<td bgcolor='#FFFFFF' class='PequenioAzulOscuro'>{$row_poa_dpto['justificacion']}&nbsp;</td>";
								echo "<td bgcolor='#FFFFFF' class='PequenioAzulOscuro'><a href='index.php?sec=4&mod=1&poa_dpto={$row_poa_dpto['poa_id']}'><img src='imagenes/pencil.png' border='0'></td>";
								echo "<td bgcolor='#FFFFFF' class='PequenioAzulOscuro'><a href='index.php?sec=d&table=poa_dpto&id={$row_poa_dpto['poa_id']}'><img src='imagenes/bin_closed.png' border='0'></a></td>";
								echo "</tr>";
							}
						}
