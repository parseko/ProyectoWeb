	...
	if ( $row_poa = mysql_fetch_assoc ( $res_poa ) )
	{
		if ( $row_poa['iniciado'] == 1 )
		{
			...
			if ( $_GET['dep'] != 1 and $_GET['dep'] != 2)
			{
				...
				{	
					...
					while ( $row_dpto = mysql_fetch_assoc ( $res_dpto ) )
					{
						...
						echo "<td class='PoaDatos'><a href='index.php?sec=6&dep=1&id={$row_dpto['id']}'><img src='imagenes/book_open.gif' border='0'></a></td>";
			...
			if ( $_POST or $_GET['dep'] == 1)
			{
				...
				$sql_programa = "SELECT * FROM poa_dpto WHERE dpto_id='{$_SESSION['DPTO']}' GROUP BY idacciones";
				if ( $res_programa = mysql_db_query ( $bdd, $sql_programa ) )
				{
					while ( $row_programa = mysql_fetch_assoc ( $res_programa ) )
					{
							$sql_gastos_dpto = "SELECT * FROM gastos_dpto WHERE iddpto='{$_SESSION['DPTO']}' AND idmeta = '{$row_programa['idacciones']}' AND donde = 'Remanente' ";
