	...
	if ( $row_poa = mysql_fetch_assoc ( $res_poa ) )
	{
		if ( $row_poa['iniciado'] == 1 )
		{
			...
			if ( isset ( $_POST['modificar'] ) )
			{
				...
				$sql_poa1 = "SELECT * FROM poa_dpto WHERE id = {$_POST['poa_dpto']}";
				$res_poa1 = mysql_db_query ( $bdd, $sql_poa1 );
				$row_poa1 = mysql_fetch_assoc ( $res_poa1 );
				{
				
				}

				$sql_insumo = "SELECT * FROM insumo WHERE id = {$row_poa1['insumo_id']}";
				$res_insumo = mysql_db_query ( $bdd, $sql_insumo );
				$row_insumo = mysql_fetch_assoc ( $res_insumo );
				{
					echo $row_insumo['costuni'];
				}
				...
				$anterior = $row_insumo['costuni'] * $row_poa1['cantidad'];
				...
				else
				{
					...
					$sql = "UPDATE poa_dpto SET partida_id = {$_POST['partida']}, insumo_id = {$_POST['insumo']}, cantidad = {$_POST['cantidad']}, descripcion = '{$_POST['descripcion']}', justificacion = '{$_POST['justificacion']}', tipogasto = '{$tipo}', periodo = '{$perio}' WHERE id = {$_POST['poa_dpto']}";
			...
			if ( $_POST or $_GET['mod'] == 1)
			{
				...
				if ( isset ( $_GET['poa_dpto'] ) and !isset ( $_POST['poa_dpto'] ) )
				{
					...
					$_POST['descripcion'] = $row_poa['descripcion'];
					$_POST['justificacion'] = $row_poa['justificacion'];
					$_POST['TipoGastos'] = $row_poa['tipogasto'];
			...
			<?php
									if ( $_POST['partida'] )
									{
										...
										if ( $capitulo == 5 )
										{
											...
											echo "<td align='left'><textarea name='descripcion'>{$_POST['descripcion']}</textarea></td>";
										}
									}
			?>
								</tr>
			...
			else
			{
				...
				if ( $res_programa = mysql_db_query ( $bdd, $sql_programa ) )
				{
					...
					{
						/*$sql_unidad = "SELECT * FROM accion WHERE claveAccion = '{$row_programa['claveAccion']}'";
						$res_unidad = mysql_db_query ( $bdd, $sql_unidad );
						if ($row_unidad = mysql_fetch_assoc ( $res_unidad ))
						{
						}
						$sql_unidad1 = "SELECT * FROM metas WHERE idmeta = '{$row_unidad['id']}' AND iddpto={$_SESSION['dpto_id']}";
						...
						echo "<tr>";
						echo "<td class='PoaUnidad' colspan='11'><strong>{$row_unidad['claveproceso']}.{$row_unidad1['claveActiv']}.{$row_unidad2['claveAccion']} </br> {$row_unidad4['accion']}</strong></td>";
						echo "</tr>";
						echo "<tr>";
						...
						echo "<td class='PoaTitulo'> GD - IP </td>";
						echo "<td class='PoaTitulo'>&nbsp;</td>";
						echo "<td class='PoaTitulo'>&nbsp;</td>";
						echo "</tr>";
						
						$sql_poa_dpto = "SELECT poa_dpto.id AS poa_id, poa_dpto.*, insumo.*, partida.* FROM poa_dpto, insumo, partida WHERE poa_dpto.dpto_id = {$_SESSION['dpto_id']}  AND poa_dpto.insumo_id = insumo.id AND poa_dpto.partida_id = partida.id AND poa_dpto.idaccion = {$row_programa['idaccion']} ORDER BY   poa_dpto.idacciones";
						if ( $res_poa_dpto = mysql_db_query ( $bdd, $sql_poa_dpto ) )
						{
							...
							while ( $row_poa_dpto = mysql_fetch_assoc ( $res_poa_dpto ) )
							{
								...
								if($row_poa_dpto['tipogasto'] == 1)
								{
									echo "<td class='PoaDatos'>IP&nbsp;</td>";
								}
								else
								{
									echo "<td class='PoaDatos'>GD&nbsp;</td>";
								}
