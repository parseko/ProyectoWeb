								...
								if ( $row_poa = mysql_fetch_assoc ( $res_poa ) )
								{
									if($row_poa['terminado']==0)
									{
										...
										while ( $row_metas = mysql_fetch_assoc ( $res_metas ) )
										{
											...
											echo "<td align='center' bgcolor='#FFFFFF' class='Menu2'><a class='PequenioAzulOscuro' href='index.php?sec=3&Acciones={$row_meta['id']}&Accion={$row_metas['idaccion']}&Proceso={$row_proceso['id']}&Clave={$row_clave['id']}'>{$row_meta1['accion']}</a></td>";
											echo "</tr>";
										}
									}
									else
									{
										echo "<div align='center' class='MedianoAlerta'>Se a Terminado el Periodo de Captura</div>";
									}
								}
