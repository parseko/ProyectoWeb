		...
		if ( $row_poa['iniciado'] == 1 and $row_poa['terminado'] == 0)
		{
			...
			{	
				...
				function Presupuesto1($dpto_id,$bdd,$partida,$meta)
				{
					...
					$sql_poa_dpto = "SELECT gastos_poa_dpto.id AS poa_id, gastos_poa_dpto.*, insumo.*, partida.* FROM gastos_poa_dpto, insumo, partida WHERE gastos_poa_dpto.dpto_id = '{$dpto_id}' AND gastos_poa_dpto.insumo_id = insumo.id AND gastos_poa_dpto.partida_id = partida.id AND gastos_poa_dpto.idacciones='$meta' AND partida.clavepartida>='{$_SESSION['inicio']}' AND partida.clavepartida<'{$_SESSION['fin']}' ";
					if ( $res_poa_dpto = mysql_db_query ( $bdd, $sql_poa_dpto ) )
					{
						$totalUnidad = 0;
						$totalgastos = 0;
				...
				function totalGastado1($dpto_id,$bdd,$partida,$meta)
				{
					...
					/*echo " "; echo $dpto_id; echo " ";
					$sql_gastos_dpto = "SELECT * FROM gastos_dpto WHERE iddpto = '{$dpto_id}' AND idmeta='{$meta}' AND donde != 'Remanente' AND donde != 'Cancelado'";
					if ( $res_gastos_dpto = mysql_db_query ( $bdd, $sql_gastos_dpto ) )
					{
						while ( $row_gastos_dpto = mysql_fetch_assoc ( $res_gastos_dpto ) )
						{
							$sql_par = "SELECT * FROM partida WHERE id = '{$row_gastos_dpto['idpartida']}' ";
							$res_par = mysql_db_query ( $bdd, $sql_par );
							while ( $row_par = mysql_fetch_assoc ( $res_par ) )
							{
								
								if( $row_par['clavepartida']>=$_SESSION['inicio'] and $row_par['clavepartida']<$_SESSION['fin'])
								{
									$total += $row_gastos_dpto['monto'];
									$totalUnidad += $total;
								}
					...
					$sql_gastos_dpto = "SELECT * FROM gastos_dpto WHERE iddpto='{$dpto_id}' AND idmeta = '{$meta}'";
					if ( $res_gastos_dpto = mysql_db_query ( $bdd, $sql_gastos_dpto ) )
					{
						while ( $row_gastos_dpto = mysql_fetch_assoc ( $res_gastos_dpto ) )
						{
