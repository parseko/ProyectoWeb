<?php
	...
	if ( $row_poa = mysql_fetch_assoc ( $res_poa ) )
	{
		if ( $row_poa['iniciado'] == 1 and $row_poa['terminado'] == 0 )
		{
			...
			{
				if ( $_POST['Accion'] == "" or $_POST['Meta'] == "" )
				{
					$error = 3;
				}
				else
				{	
					$sql = "INSERT INTO metas ( iddpto, idmeta, idpoa, idpreacciones ) VALUES ( {$_SESSION['dpto_id']}, {$_POST['Meta']},  {$row_poa['id']} , {$_POST['Accion']} )";
						...
						<table width="100%" align="center" cellpadding="3" cellspacing="3" class="MedianoAzul">
							<tr>
								...
								<td align="left" width="65%" class="MedianoAzulOscuro">
									<select name="Meta" class="ControlesTexto" onchange="submit()" >
		...
		<?php
						...
						{
		?>
										<option value="<?php echo $row_proyecto['id']; ?>" <?php if ( $_POST['Meta'] == $row_proyecto['id'] ) { ?> selected="selected" <?php } ?> > <?php echo $row_proyecto['claveAccion']; ?> .- <?php echo $row_proyecto['nombreAccion']; ?> </option>
		<?php
						}
		?>
								  </select>								</td>
							...
							<tr>
							  ...
							  <?php 
								...
								if ( $_POST['Meta'] != 0 )
								{
									$sql_accion = "SELECT * FROM accion WHERE id = '{$_POST['Meta']}'";
							...
							<tr>
								...
								{
									?>
									<select name="Accion" class="ControlesTexto" id="Accion" onchange="submit()">
									  <option >[Seleccione una opci&oacute;n]</option>
									  <?php
									  
										$sql_preacciones = "SELECT * FROM preacciones WHERE idmeta = '{$_POST['Meta']}'";
										$res_preacciones = mysql_db_query ( $bdd, $sql_preacciones );
										while ( $row_preacciones = mysql_fetch_assoc ( $res_preacciones ) )
										{
						?>
												  <option value="<?php echo $row_preacciones['id']; ?>" <?php if ( $_POST['Accion'] == $row_preacciones['id'] ) { ?> selected="selected" <?php } ?> > <?php echo $row_preacciones['claveaccion']; ?> .- <?php echo $row_preacciones['accion']; ?> </option>
												  <?php
										}
		?>
                                	</select>
							...
							?>
										<td colspan="3" align="center" class="PequenioAzul">
											<input type="submit" name="ingresar"  value="Cargar"/>										</td>
							<?php
								}
							?>
							</tr>
