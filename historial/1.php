<?php
	...
	{	
		$sql_propios = "SELECT * FROM historial WHERE claveproceso = '{$row_proyecto['claveproceso']}' AND claveActiv = {$row_proyecto['claveActiv']} AND claveAccion = {$row_proyecto['claveAccion']} AND anio = {$row_ejercicio['anio']} AND ejercicio = {$row_ejercicio['tipo']}";
		$res_propiosP = mysql_db_query ( $bdd, $sql_propios );
		
		$sql_operacional = "SELECT * FROM historial_federal WHERE claveproceso = '{$row_proyecto['claveproceso']}' AND claveActiv = {$row_proyecto['claveActiv']} AND claveAccion = {$row_proyecto['claveAccion']} AND anio = {$row_ejercicio['anio']} AND ejercicio = {$row_ejercicio['tipo']}";
		...
		if ( $row_propiosP = mysql_fetch_assoc ( $res_propiosP ) or $row_operacionalP = mysql_fetch_assoc ( $res_operacionalP ) )
		{
			...
			while ( $row_partida = mysql_fetch_assoc ( $res_partida ) )
			{
				...
				$sql_propios = "SELECT * FROM historial WHERE claveproceso = '{$row_proyecto['claveproceso']}' AND claveActiv = {$row_proyecto['claveActiv']} AND claveAccion = {$row_proyecto['claveAccion']} AND clavepartida = {$row_partida['clavepartida']} AND anio = {$row_ejercicio['anio']} AND ejercicio = {$row_ejercicio['tipo']}";
				$res_propios = mysql_db_query ( $bdd, $sql_propios );
				while ( $row_propios = mysql_fetch_assoc ( $res_propios ) )
				{
					$propios += ( $row_propios['costuni']*$row_propios['cantidad'] );
				}
				
				$sql_operacional = "SELECT * FROM historial_federal WHERE claveproceso = '{$row_proyecto['claveproceso']}' AND claveActiv = {$row_proyecto['claveActiv']} AND claveAccion = {$row_proyecto['claveAccion']} AND clavepartida = {$row_partida['clavepartida']} AND anio = {$row_ejercicio['anio']} AND ejercicio = {$row_ejercicio['tipo']}";
