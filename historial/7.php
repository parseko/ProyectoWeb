<?php
	...
	if ( $res_programa = mysql_db_query ( $bdd, $sql_programa ) )
	{
		while ( $row_programa = mysql_fetch_assoc ( $res_programa ) )
		{
			...
			$sql_poa_dpto = "SELECT * FROM historial WHERE claveproceso = '{$row_programa['claveproceso']}' AND claveActiv = {$row_programa['claveActiv']} AND claveAccion = {$row_programa['claveAccion']} AND tipounidad = '{$row_programa['tipounidad']}' AND anio = {$row_ejercicio['anio']} AND ejercicio = {$row_ejercicio['tipo']} AND dpto_id = {$_POST['dpto_id']} ORDER BY clavepartida, descinsu";
